  ...
  function createVMOpt(node, kind) {
    var opt = {
      filename:
        ...
        ":" +
        node.id +
        (node.name ? " [" + node.name + "]" : ""), // filename for stack traces
      displayErrors: true,
      ...
      modules.forEach((module) => {
        ...
        if (vname && vname !== "") {
          if (sandbox.hasOwnProperty(vname) || vname === "node") {
            node.error(
              RED._("function.error.moduleNameError", { name: vname })
            );
            moduleErrors = true;
            return;
          }
          ...
          if (spec && spec !== "") {
            moduleLoadPromises.push(
              RED.import(module.module)
                ...
                .catch((err) => {
                  node.error(
                    RED._("function.error.moduleLoadError", {
                      module: module.spec,
                      error: err.toString(),
                    })
                  );
    ...
    async function (req, res) {
      ...
      if (node != null) {
        try {
          ...
          const returnMsg =
            req.body.returnMsg === false || req.body.returnMsg === "false"
              ? false
              : true;
          if (
            req.body.config &&
            (req.body.config.credentials || req.body.config.model)
          ) {
            config = {};
          ...
          let errCode =
            ...
            err.response.data.error.code
              ? err.response.data.error.code
              : "unknown_error";
          let statusMessage = err.response
